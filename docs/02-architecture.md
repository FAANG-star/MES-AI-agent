...
Steps 1–5 are tool calls and are what `Understanding.plan` contains — the
planner produces them, the executor runs them. Steps 6–8 are not tool calls
and never appear in the plan: the ranking is `kind: "engine"`, the
explanation is `kind: "llm"`, and the validation is
...
**ADR-1 — A fixed, inspectable plan rather than free-form ReAct.** *(amended)*
...
**ADR-3 — Math outside the LLM.** *(implemented)*
...
**ADR-5 — Provider-abstracted LLM, local-first.** *(implemented)*
...
**ADR-9 — Tool selection: model-proposed, template-guaranteed, registry-validated.** *(amended)*
The model is given the live tool catalogue and names the tools a question needs. A per-intent template then guarantees the sequence each demo scenario depends on, in an order where every step's inputs exist; unrecognised names are dropped and logged. A purely model-chosen plan varies run to run — untestable against the fixed sequences in `04-demo-scenarios.md` and unsafe in a live demo — while a purely fixed plan would not be an agent. This keeps the judgement with the model and the reproducibility with the code.

**Amendment: tools the model proposes beyond the template are recorded, not run.** The live scenario matrix showed the local model adding `get_production_history` to every bottleneck question. Nothing in a bottleneck answer reads it, so the call bought nothing and cost two things: "Data Used" listed a table the answer never used (acceptance criterion 5 is about *which information was used*), and every figure it returned became a number the validator would accept in the prose — a wider door for grounded-but-wrong sentences. The proposal is kept in the run's notes, so the model's judgement stays auditable. The model's *intent* reading is also now corrected for the two confusions the matrix observed (see [`07-agent-understanding.md`](07-agent-understanding.md) and [`12-testing.md`](12-testing.md)).

**ADR-6 — Read-only DB role for the tool layer.** *(implemented)*
Tools connect as `mes_ro`: SELECT grants only, plus `default_transaction_read_only`, so writes fail at the transaction level even if a grant were wrong. The role is created idempotently by `db/schema.sql`, and `backend/tests/test_readonly.py` asserts DELETE/UPDATE/INSERT/DROP are all refused on the live connection. Tightened from the original sketch: `mes_ro` has **no** INSERT on `agent_run_log` — the audit trail is written over the application's own read-write connection, leaving the tool path with no write capability at all.
...
│  └─ verify.sql             20 data assertions / SQL oracle
├─ backend/
│  ├─ app/
...
│  │  │                      understanding · executor · pipeline ·
│  │  │                      tracing · derive ·
│  │  │                      explainer · validator · answering
...
│  ├─ tests/                 59 unit tests: sse · steps · run stage · formatting · theme · time zones
│  ├─ e2e/                   10 browser tests against the running stack (Playwright)
...
| `GET` | `/api/tools` | tool catalogue with LLM-ready JSON schemas | ✅ |
| `GET` | `/api/tools/{name}` | one tool's contract | ✅ |
| `POST` | `/api/tools/{name}` | invoke a MES tool | ✅ |
| `POST` | `/api/understand` | question → guard, rewrite, typed intent, entities, plan (nothing executed) | ✅ |
| `GET` | `/api/agent` | active LLM provider, intents, pipeline stages | ✅ |
| `POST` | `/api/ask` | question → the complete structured run | ✅ |
| `POST` | `/api/ask/stream` | the same run as SSE: `accepted`, `understanding`, `tool_result`, `answer`, `error`, `run` | ✅ |
